 ...
 * @retval false If \p file could not be opened.
 */
bool DL_Dxf::in(std::istream& stream, DL_CreationInterface* creationInterface) {
    if (stream.good()) {
        firstCall = true;
        currentObjectType = DL_UNKNOWN;
...
bool DL_Dxf::readDxfGroups(FILE* fp, DL_CreationInterface* creationInterface) {
    ...
    if (DL_Dxf::getStrippedLine(groupCodeTmp, DL_DXF_MAXLINE, fp) && DL_Dxf::getStrippedLine(groupValue, DL_DXF_MAXLINE, fp, false)) {
 ...
 */
bool DL_Dxf::readDxfGroups(std::istream& stream, DL_CreationInterface* creationInterface) {
    static int line = 1;

    // Read one group of the DXF file and chop the lines:
    if (DL_Dxf::getStrippedLine(groupCodeTmp, DL_DXF_MAXLINE, stream) && DL_Dxf::getStrippedLine(groupValue, DL_DXF_MAXLINE, stream, false)) {
        groupCode = (unsigned int)toInt(groupCodeTmp);
 ...
 * Same as above but for input streams.
 */
bool DL_Dxf::getStrippedLine(std::string& s, unsigned int size, std::istream& stream, bool stripSpace) {
    if (!stream.eof()) {
        // Only the useful part of the line
...
bool DL_Dxf::stripWhiteSpace(char** s, bool stripSpace) {
    ...
    while ((lastChar >= 0) &&
           (((*s)[lastChar] == 10) || ((*s)[lastChar] == 13) || (stripSpace && ((*s)[lastChar] == ' ' || ((*s)[lastChar] == '\t'))))) {
        (*s)[lastChar] = '\0';
        lastChar--;
    }
 ...
 */
bool DL_Dxf::processDXFGroup(DL_CreationInterface* creationInterface, int groupCode, const std::string& groupValue) {
    // printf("%d\n", groupCode);
    // printf("%s\n", groupValue.c_str());
    ...
    // Indicates start of new entity or variable:
        ...
        creationInterface->setExtrusion(getRealValue(210, 0.0), getRealValue(220, 0.0), getRealValue(230, 1.0),
 ...
 */
void DL_Dxf::addComment(DL_CreationInterface* creationInterface, const std::string& comment) { creationInterface->addComment(comment); }

void DL_Dxf::addDictionary(DL_CreationInterface* creationInterface) { creationInterface->addDictionary(DL_DictionaryData(getStringValue(5, ""))); }
...
void DL_Dxf::addSetting(DL_CreationInterface* creationInterface) {
    ...
    // vector
        ...
        if (c == 10) {
            creationInterface->setVariableVector(settingKey, getRealValue(c, 0.0), getRealValue(c + 10, 0.0), getRealValue(c + 20, 0.0), c);
        }
    }
    // double
    else if (c >= 40 && c <= 59) {
 ...
 */
void DL_Dxf::endBlock(DL_CreationInterface* creationInterface) { creationInterface->endBlock(); }
...
void DL_Dxf::addPoint(DL_CreationInterface* creationInterface) {
    DL_PointData d(getRealValue(10, 0.0), getRealValue(20, 0.0), getRealValue(30, 0.0));
...
void DL_Dxf::addLine(DL_CreationInterface* creationInterface) {
    DL_LineData d(getRealValue(10, 0.0), getRealValue(20, 0.0), getRealValue(30, 0.0), getRealValue(11, 0.0), getRealValue(21, 0.0),
                  getRealValue(31, 0.0));
...
void DL_Dxf::addXLine(DL_CreationInterface* creationInterface) {
    DL_XLineData d(getRealValue(10, 0.0), getRealValue(20, 0.0), getRealValue(30, 0.0), getRealValue(11, 0.0), getRealValue(21, 0.0),
                   getRealValue(31, 0.0));
...
void DL_Dxf::addRay(DL_CreationInterface* creationInterface) {
    DL_RayData d(getRealValue(10, 0.0), getRealValue(20, 0.0), getRealValue(30, 0.0), getRealValue(11, 0.0), getRealValue(21, 0.0),
                 getRealValue(31, 0.0));
...
void DL_Dxf::addVertex(DL_CreationInterface* creationInterface) {
    ...
    DL_VertexData d(getRealValue(10, 0.0), getRealValue(20, 0.0), getRealValue(30, 0.0), getRealValue(42, 0.0));
...
void DL_Dxf::addSpline(DL_CreationInterface* creationInterface) {
    DL_SplineData sd(getIntValue(71, 3), maxKnots, maxControlPoints, maxFitPoints, getIntValue(70, 4));
    ...
    for (i = 0; i < maxControlPoints; i++) {
        DL_ControlPointData d(controlPoints[i * 3], controlPoints[i * 3 + 1], controlPoints[i * 3 + 2], weights[i]);

        creationInterface->addControlPoint(d);
    }
    for (i = 0; i < maxFitPoints; i++) {
        DL_FitPointData d(fitPoints[i * 3], fitPoints[i * 3 + 1], fitPoints[i * 3 + 2]);
...
void DL_Dxf::addArc(DL_CreationInterface* creationInterface) {
    DL_ArcData d(getRealValue(10, 0.0), getRealValue(20, 0.0), getRealValue(30, 0.0), getRealValue(40, 0.0), getRealValue(50, 0.0),
                 getRealValue(51, 0.0));
...
void DL_Dxf::addCircle(DL_CreationInterface* creationInterface) {
    DL_CircleData d(getRealValue(10, 0.0), getRealValue(20, 0.0), getRealValue(30, 0.0), getRealValue(40, 0.0));
...
void DL_Dxf::addEllipse(DL_CreationInterface* creationInterface) {
    DL_EllipseData d(getRealValue(10, 0.0), getRealValue(20, 0.0), getRealValue(30, 0.0), getRealValue(11, 0.0), getRealValue(21, 0.0),
                     getRealValue(31, 0.0), getRealValue(40, 1.0), getRealValue(41, 0.0), getRealValue(42, 2 * M_PI));
...
bool DL_Dxf::handleXRecordData(DL_CreationInterface* creationInterface) {
    ...
    // string:
    if (groupCode <= 9 || groupCode == 100 || groupCode == 102 || groupCode == 105 || (groupCode >= 300 && groupCode <= 369) ||
...
bool DL_Dxf::handleLWPolylineData(DL_CreationInterface* /*creationInterface*/) {
    ...
    else if (groupCode == 10 || groupCode == 20 || groupCode == 30 || groupCode == 42) {
...
bool DL_Dxf::handleLeaderData(DL_CreationInterface* /*creationInterface*/) {
    ...
    // Process Leader vertices (group codes 10/20/30):
        ...
        if (groupCode <= 30) {
            if (leaderVertexIndex >= 0 && leaderVertexIndex < maxLeaderVertices) {
                leaderVertices[3 * leaderVertexIndex + (groupCode / 10 - 1)] = toReal(groupValue);
            }
        }
        return true;
    }
...
void DL_Dxf::addText(DL_CreationInterface* creationInterface) {
    DL_TextData d(
        ...
        getRealValue(10, 0.0), getRealValue(20, 0.0), getRealValue(30, 0.0),
        // alignment point
        getRealValue(11, DL_NANDOUBLE), getRealValue(21, DL_NANDOUBLE), getRealValue(31, DL_NANDOUBLE),
        // height
        getRealValue(40, 2.5),
        // x scale
        getRealValue(41, 1.0),
...
void DL_Dxf::addDimAngular(DL_CreationInterface* creationInterface) {
    ...
    DL_DimAngular2LData da(
        ...
        getRealValue(15, 0.0), getRealValue(25, 0.0), getRealValue(35, 0.0),
        // definition point 4
        getRealValue(16, 0.0), getRealValue(26, 0.0), getRealValue(36, 0.0));
...
void DL_Dxf::addDimOrdinate(DL_CreationInterface* creationInterface) {
    ...
    DL_DimOrdinateData dl(
        ...
        getRealValue(13, 0.0), getRealValue(23, 0.0), getRealValue(33, 0.0),
        // definition point 2
        getRealValue(14, 0.0), getRealValue(24, 0.0), getRealValue(34, 0.0), (getIntValue(70, 0) & 64) == 64  // true: X-type, false: Y-type
    );
...
void DL_Dxf::addLeader(DL_CreationInterface* creationInterface) {
    ...
    for (int i = 0; i < maxLeaderVertices; i++) {
        DL_LeaderVertexData d(leaderVertices[i * 3], leaderVertices[i * 3 + 1], leaderVertices[i * 3 + 2]);
...
void DL_Dxf::addHatch(DL_CreationInterface* creationInterface) {
    DL_HatchData hd(getIntValue(91, 1), getIntValue(70, 0), getRealValue(41, 1.0), getRealValue(52, 0.0), getStringValue(2, ""));
 ...
 */
void DL_Dxf::endEntity(DL_CreationInterface* creationInterface) { creationInterface->endEntity(); }
 ...
 */
void DL_Dxf::writePoint(DL_WriterA& dw, const DL_PointData& data, const DL_Attributes& attrib) {
    dw.entity("POINT");
    if (version == DL_VERSION_2000) {
 ...
 * @param attrib Attributes
 */
void DL_Dxf::writeXLine(DL_WriterA& dw, const DL_XLineData& data, const DL_Attributes& attrib) {
    dw.entity("XLINE");
    if (version == DL_VERSION_2000) {
        dw.dxfString(100, "AcDbEntity");
    }
 ...
 * @param attrib Attributes
 */
void DL_Dxf::writeRay(DL_WriterA& dw, const DL_RayData& data, const DL_Attributes& attrib) {
    dw.entity("RAY");
    if (version == DL_VERSION_2000) {
        dw.dxfString(100, "AcDbEntity");
    }
 ...
 * @see writeVertex
 */
void DL_Dxf::writePolyline(DL_WriterA& dw, const DL_PolylineData& data, const DL_Attributes& attrib) {
    if (version == DL_VERSION_2000) {
 ...
 */
void DL_Dxf::writeSpline(DL_WriterA& dw, const DL_SplineData& data, const DL_Attributes& attrib) {
    dw.entity("SPLINE");
    if (version == DL_VERSION_2000) {
        dw.dxfString(100, "AcDbEntity");
    }
 ...
 * @param attrib Attributes
 */
void DL_Dxf::writeControlPoint(DL_WriterA& dw, const DL_ControlPointData& data) {
    dw.dxfReal(10, data.x);
    dw.dxfReal(20, data.y);
 ...
 * @param attrib Attributes
 */
void DL_Dxf::writeCircle(DL_WriterA& dw, const DL_CircleData& data, const DL_Attributes& attrib) {
    dw.entity("CIRCLE");
    if (version == DL_VERSION_2000) {
        dw.dxfString(100, "AcDbEntity");
    }
 ...
 * @param attrib Attributes
 */
void DL_Dxf::writeArc(DL_WriterA& dw, const DL_ArcData& data, const DL_Attributes& attrib) {
    dw.entity("ARC");
    if (version == DL_VERSION_2000) {
        dw.dxfString(100, "AcDbEntity");
    }
 ...
 * @param attrib Attributes
 */
void DL_Dxf::writeEllipse(DL_WriterA& dw, const DL_EllipseData& data, const DL_Attributes& attrib) {
 ...
 * @param attrib Attributes
 */
void DL_Dxf::writeSolid(DL_WriterA& dw, const DL_SolidData& data, const DL_Attributes& attrib) {
    dw.entity("SOLID");
    if (version == DL_VERSION_2000) {
        dw.dxfString(100, "AcDbEntity");
    }
 ...
 * @param attrib Attributes
 */
void DL_Dxf::writeTrace(DL_WriterA& dw, const DL_TraceData& data, const DL_Attributes& attrib) {
    dw.entity("TRACE");
    if (version == DL_VERSION_2000) {
        dw.dxfString(100, "AcDbEntity");
    }
 ...
 * @param attrib Attributes
 */
void DL_Dxf::write3dFace(DL_WriterA& dw, const DL_3dFaceData& data, const DL_Attributes& attrib) {
    dw.entity("3DFACE");
    if (version == DL_VERSION_2000) {
        dw.dxfString(100, "AcDbEntity");
    }
 ...
 * @param attrib Attributes
 */
void DL_Dxf::writeInsert(DL_WriterA& dw, const DL_InsertData& data, const DL_Attributes& attrib) {
    if (data.name.empty()) {
        std::cerr << "DL_Dxf::writeInsert: " << "Block name must not be empty\n";
        return;
    }
 ...
 * @param attrib Attributes
 */
void DL_Dxf::writeMText(DL_WriterA& dw, const DL_MTextData& data, const DL_Attributes& attrib) {
    dw.entity("MTEXT");
    if (version == DL_VERSION_2000) {
        dw.dxfString(100, "AcDbEntity");
    }
 ...
 * @param attrib Attributes
 */
void DL_Dxf::writeText(DL_WriterA& dw, const DL_TextData& data, const DL_Attributes& attrib) {
    dw.entity("TEXT");
    if (version == DL_VERSION_2000) {
        dw.dxfString(100, "AcDbEntity");
    }
...
void DL_Dxf::writeAttribute(DL_WriterA& dw, const DL_AttributeData& data, const DL_Attributes& attrib) {
    dw.entity("ATTRIB");
    if (version == DL_VERSION_2000) {
        dw.dxfString(100, "AcDbEntity");
    }
...
void DL_Dxf::writeDimStyleOverrides(DL_WriterA& dw, const DL_DimensionData& data) {
    if (version == DL_VERSION_2000) {
 ...
 * @param attrib Attributes
 */
void DL_Dxf::writeDimAligned(DL_WriterA& dw, const DL_DimensionData& data, const DL_DimAlignedData& edata, const DL_Attributes& attrib) {
    dw.entity("DIMENSION");
 ...
 * @param attrib Attributes
 */
void DL_Dxf::writeDimLinear(DL_WriterA& dw, const DL_DimensionData& data, const DL_DimLinearData& edata, const DL_Attributes& attrib) {
    dw.entity("DIMENSION");
 ...
 * @param attrib Attributes
 */
void DL_Dxf::writeDimRadial(DL_WriterA& dw, const DL_DimensionData& data, const DL_DimRadialData& edata, const DL_Attributes& attrib) {
    dw.entity("DIMENSION");
 ...
 * @param attrib Attributes
 */
void DL_Dxf::writeDimDiametric(DL_WriterA& dw, const DL_DimensionData& data, const DL_DimDiametricData& edata, const DL_Attributes& attrib) {
    dw.entity("DIMENSION");
 ...
 * @param attrib Attributes
 */
void DL_Dxf::writeDimAngular2L(DL_WriterA& dw, const DL_DimensionData& data, const DL_DimAngular2LData& edata, const DL_Attributes& attrib) {
    dw.entity("DIMENSION");
 ...
 * @param attrib Attributes
 */
void DL_Dxf::writeDimAngular3P(DL_WriterA& dw, const DL_DimensionData& data, const DL_DimAngular3PData& edata, const DL_Attributes& attrib) {
    dw.entity("DIMENSION");
 ...
 * @param attrib Attributes
 */
void DL_Dxf::writeDimOrdinate(DL_WriterA& dw, const DL_DimensionData& data, const DL_DimOrdinateData& edata, const DL_Attributes& attrib) {
    dw.entity("DIMENSION");
 ...
 * @see writeVertex
 */
void DL_Dxf::writeLeader(DL_WriterA& dw, const DL_LeaderData& data, const DL_Attributes& attrib) {
    if (version > DL_VERSION_R12) {
        dw.entity("LEADER");
        if (version == DL_VERSION_2000) {
            dw.dxfString(100, "AcDbEntity");
 ...
 */
void DL_Dxf::writeLeaderVertex(DL_WriterA& dw, const DL_LeaderVertexData& data) {
    if (version > DL_VERSION_R12) {
        dw.dxfReal(10, data.x);
        dw.dxfReal(20, data.y);
    }
 ...
 * @param attrib Attributes
 */
void DL_Dxf::writeHatch1(DL_WriterA& dw, const DL_HatchData& data, const DL_Attributes& attrib) {
    dw.entity("HATCH");
    if (version == DL_VERSION_2000) {
        dw.dxfString(100, "AcDbEntity");
    }
 ...
 * @param attrib Attributes
 */
void DL_Dxf::writeHatch2(DL_WriterA& dw, const DL_HatchData& data, const DL_Attributes& /*attrib*/) {
 ...
 * @param attrib Attributes
 */
void DL_Dxf::writeHatchLoop2(DL_WriterA& dw, const DL_HatchLoopData& /*data*/) { dw.dxfInt(97, 0); }
 ...
 */
unsigned long DL_Dxf::writeImage(DL_WriterA& dw, const DL_ImageData& data, const DL_Attributes& attrib) {
    /*if (data.file.empty()) {
 ...
 */
void DL_Dxf::writeImageDef(DL_WriterA& dw, int handle, const DL_ImageData& data) {
    /*if (data.file.empty()) {
        std::cerr << "DL_Dxf::writeImage: "
        << "Image file must not be empty\n";
        return;
 ...
 * @param attrib Attributes
 */
void DL_Dxf::writeLayer(DL_WriterA& dw, const DL_LayerData& data, const DL_Attributes& attrib) {
    if (data.name.empty()) {
        std::cerr << "DL_Dxf::writeLayer: " << "Layer name must not be empty\n";
        return;
    }
    ...
    dw.dxfString(6, (attrib.getLinetype().length() == 0 ? std::string("CONTINUOUS") : attrib.getLinetype()));
...
void DL_Dxf::writeLinetype(DL_WriterA& dw, const DL_LinetypeData& data) {
    ...
    if (data.name.empty()) {
        std::cerr << "DL_Dxf::writeLinetype: " << "Line type name must not be empty\n";
        return;
    }
...
void DL_Dxf::writeAppid(DL_WriterA& dw, const std::string& name) {
    if (name.empty()) {
        std::cerr << "DL_Dxf::writeAppid: " << "Application  name must not be empty\n";
        return;
    }
...
void DL_Dxf::writeBlock(DL_WriterA& dw, const DL_BlockData& data) {
    if (data.name.empty()) {
        std::cerr << "DL_Dxf::writeBlock: " << "Block name must not be empty\n";
        return;
    }
 ...
 * to make the file readable by Aut*cad.
 */
void DL_Dxf::writeDimStyle(DL_WriterA& dw, double dimasz, double dimexe, double dimexo, double dimgap, double dimtxt) {
    dw.dxfString(0, "TABLE");
...
int DL_Dxf::getLibVersion(const std::string& str) {
    ...
    if (idx >= 2) {
        ...
        ret = (atoi(v[0].c_str()) << (3 * 8)) + (atoi(v[1].c_str()) << (2 * 8)) + (atoi(v[2].c_str()) << (1 * 8)) + (atoi(v[3].c_str()) << (0 * 8));
...
void DL_Dxf::test() {
    ...
    PLOG_INFO << "1 buf1: '" << buf1 << "'\n";
    stripWhiteSpace(&buf1);
    PLOG_INFO << "2 buf1: '" << buf1 << "'\n";
    // assert(!strcmp(buf1, "10"));

    PLOG_INFO << "1 buf2: '" << buf2 << "'\n";
    stripWhiteSpace(&buf2);
    PLOG_INFO << "2 buf2: '" << buf2 << "'\n";

    PLOG_INFO << "1 buf3: '" << buf3 << "'\n";
    stripWhiteSpace(&buf3);
    PLOG_INFO << "2 buf3: '" << buf3 << "'\n";

    PLOG_INFO << "1 buf4: '" << buf4 << "'\n";
    stripWhiteSpace(&buf4);
    PLOG_INFO << "2 buf4: '" << buf4 << "'\n";

    PLOG_INFO << "1 buf5: '" << buf5 << "'\n";
    stripWhiteSpace(&buf5);
    PLOG_INFO << "2 buf5: '" << buf5 << "'\n";

    PLOG_INFO << "1 buf6: '" << buf6 << "'\n";
    stripWhiteSpace(&buf6);
    PLOG_INFO << "2 buf6: '" << buf6 << "'\n";
}
