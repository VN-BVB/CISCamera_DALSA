...
void TinySplineqqq::runcv() {
    ...
    for (size_t i = 0; i < imageControlPoints.size() - 1; ++i) {
        cv::line(image, imageControlPoints[i], imageControlPoints[i + 1], cv::Scalar(255, 0, 0), 1, cv::LINE_AA);
    }
    ...
    for (size_t i = 0; i < splinePoints.size() - 1; ++i) {
        cv::line(image, splinePoints[i], splinePoints[i + 1], cv::Scalar(0, 255, 0), 2, cv::LINE_AA);
    }
    ...
    for (float u = 0.0f; u <= 1.0f; u += 0.2f) {
        ...
        cv::Point img_tangent_end = mathToImage(point[0] + tangent[0] * 20.0f,  // 放大切线长度以便显示
                                                point[1] + tangent[1] * 20.0f);

        cv::arrowedLine(image, img_point, img_tangent_end, cv::Scalar(255, 0, 255), 2, cv::LINE_AA);
    }

    // 添加图例和标签
    cv::putText(image, "控制点 (红色)", cv::Point(20, 30), cv::FONT_HERSHEY_SIMPLEX, 0.6, cv::Scalar(0, 0, 255), 2);
    cv::putText(image, "控制多边形 (蓝色)", cv::Point(20, 60), cv::FONT_HERSHEY_SIMPLEX, 0.6, cv::Scalar(255, 0, 0), 2);
    cv::putText(image, "样条曲线 (绿色)", cv::Point(20, 90), cv::FONT_HERSHEY_SIMPLEX, 0.6, cv::Scalar(0, 255, 0), 2);
    cv::putText(image, "切线 (紫色)", cv::Point(20, 120), cv::FONT_HERSHEY_SIMPLEX, 0.6, cv::Scalar(255, 0, 255), 2);
