...
CannyDevernay::CannyDevernay()
    : m_sigma(1.5), m_highThreshold(4.2), m_lowThreshold(0.81) {}

CannyDevernay::~CannyDevernay() {}

std::vector<Point2fCurve> CannyDevernay::detectEdges(const cv::Mat& inputImage,
                                                         double sigma,
    ...
    // 分配高斯滤波结果内存
    uchar* gauss = (uchar*)xmalloc(X * Y * sizeof(uchar));

    // 调用原始devernay函数
    devernay(&x, &y, &N, &curve_limits, &M,
             inputImage.data, gauss, X, Y,
             m_sigma, m_highThreshold, m_lowThreshold);

    // 转换结果为EdgeCurve结构
    m_edgeCurves.clear();
    for (int i = 0; i < M; i++) {
        EdgeCurve curve;
        int start = curve_limits[i];
        int end = curve_limits[i + 1];

        for (int j = start; j < end; j++) {
            EdgePoint point = {x[j], y[j]};
            curve.points.push_back(point);
        }

        // 检查曲线是否闭合
        if (curve.points.size() >= 2) {
            EdgePoint& first = curve.points.front();
            EdgePoint& last = curve.points.back();
            curve.isClosed = (dist(first.x, first.y, last.x, last.y) < 1.0);
        }

        m_edgeCurves.push_back(curve);
    }
    std::vector<Point2fCurve> Point2fCurves;
    Point2fCurves = this->convertToPoint2fCurves(m_edgeCurves);
    ...
    return Point2fCurves;
}

// 参数设置方法
void CannyDevernay::setSigma(double sigma) { m_sigma = sigma; }
void CannyDevernay::setHighThreshold(double threshold) { m_highThreshold = threshold; }
void CannyDevernay::setLowThreshold(double threshold) { m_lowThreshold = threshold; }

const std::vector<EdgeCurve>& CannyDevernay::getEdgeCurves() const {
    return m_edgeCurves;
}

// 以下是原始函数的封装实现
void CannyDevernay::error(const char* msg) {
    fprintf(stderr, "error: %s\n", msg);
    exit(EXIT_FAILURE);
}

void* CannyDevernay::xmalloc(size_t size) {
    void* p;
...
int CannyDevernay::greater_double(double a, double b) {
    if (a <= b) return 0;
    if ((a - b) < 1000 * DBL_EPSILON) return 0;
    return 1;
}

double CannyDevernay::dist(double x1, double y1, double x2, double y2) {
...
*/
void CannyDevernay::gaussian_kernel(double * pfGaussFilter, int n, double sigma, double mean)
{
    double sum = 0.0;
    double val;
    int i;
...
to a newly allocated filtered image,
of the same size as the input image. */
void CannyDevernay::gaussian_filter(uchar* image, uchar* out, int X, int Y, double sigma)
{
...
*/
double CannyDevernay::chain(int from, int to, double * Ex, double * Ey,double * Gx, double * Gy, int X, int Y)
{
    double dx, dy;
...
void CannyDevernay::compute_gradient(double * Gx, double * Gy, double * modG,uchar * image, int X, int Y)
{
    int x, y;
    ...
    // approximate image gradient using centered differences（中心差分法）
    for (x = 1; x<(X - 1); x++)
        for (y = 1; y<(Y - 1); y++)
        {
...
Devernay校正方法在仅沿水平或垂直方向进行插值时效果非常好且非常一致。
这个修改版本要求一个像素要成为边缘点，必须在水平或垂直方向上是局部最大值，
具体取决于梯度方向：如果梯度的x分量大于y分量（Gx > Gy），这意味着梯度大致是水平的，
因此需要水平方向的最大值才能成为边缘点。
*/
void CannyDevernay::compute_edge_points(double * Ex, double * Ey, double * modG,
                         double * Gx, double * Gy, int X, int Y)
{
    ...
    for (x = 2; x<(X - 2); x++)
        ...
        done using the function greater_double() instead of the operators > or >= so numbers differing only
        due to rounding errors are considered equal */
            if (greater_double(mod, L) && !greater_double(R, mod) && gx >= gy) Dx = 1; /* H */
            else if (greater_double(mod, D) && !greater_double(U, mod) && gx <= gy) Dy = 1; /* V */
...
void CannyDevernay::chain_edge_points(int * next, int * prev, double * Ex,	double * Ey,double * Gx, double * Gy, int X, int Y)
{
    int x, y, i, j, alt;
...
thresholds.
*/
void CannyDevernay::thresholds_with_hysteresis(int * next, int * prev,
...
*/
void CannyDevernay::list_chained_edge_points(double ** x, double ** y, int * N,int ** curve_limits,
...
y[curve_limits[k]] == y[curve_limits[k+1] - 1].
*/
void CannyDevernay::devernay(double ** x, double ** y, int * N, int ** curve_limits,int * M,
...
std::vector<Point2fCurve> CannyDevernay::convertToPoint2fCurves(const std::vector<EdgeCurve> &edgeCurves)
{
    std::vector<Point2fCurve> result;

    for (const auto& edgeCurve : edgeCurves) {
        Point2fCurve curve;
        curve.isClosed = edgeCurve.isClosed;

        for (const auto& edgePoint : edgeCurve.points) {
            curve.points.emplace_back(static_cast<float>(edgePoint.x),
