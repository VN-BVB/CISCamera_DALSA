...
void CameraImageProcessor::lodaCam2PlatCalibrateParams() {
    ...
    if (!allRotVecs_.empty() && allTransVecs_.size() > 1 && allTransVecs_.back().norm() > 1e-6) {
        ...
        for (int j = 0; j < 3; ++j) {
            r_wc.at<double>(j) = wR(j);
            t_wc.at<double>(j) = wT(j);
        }
        ...
        for (size_t i = 0; i + 1 < allRotVecs_.size(); ++i) {
            ...
            for (int j = 0; j < 3; ++j) {
                r_pw.at<double>(j) = allRotVecs_[i](j);
                t_pw.at<double>(j) = allTransVecs_[i](j);
            }
...
void CameraImageProcessor::whenCalibrateCP() {
    ...
    //         for (int j = 0; j < 3; ++j) {
    //             r_pc.at<double>(j) = poseData.allRotVecs[i](j);
    //             t_pc.at<double>(j) = poseData.allTransVecs[i](j);
    //         }
    //         cv::Rodrigues(r_pc, R_pc);
    //         cv::Mat R_pw = R_cw * R_pc, t_pw = R_cw * (t_pc - t_wc), r_pw;
    //         t_pw.at<double>(2) = 0;
    //         cv::Rodrigues(R_pw, r_pw);
