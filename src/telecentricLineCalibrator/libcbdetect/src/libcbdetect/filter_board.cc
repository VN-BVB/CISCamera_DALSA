...
double find_minE(const Board& board, const cv::Point2i& p) {
    double minE = std::min(std::min(board.energy[p.y][p.x][0], board.energy[p.y][p.x][1]), board.energy[p.y][p.x][2]);
    if (p.x - 1 >= 0) {
...
void filter_board(const Corner& corners, std::vector<int>& used, Board& board, std::vector<cv::Point2i>& proposal, double& energy,
                  const Params& params) {
    // erase wrong corners
