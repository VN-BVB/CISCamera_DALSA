...
void debug_grow_process(const cv::Mat& img, const Corner& corners, const Board& board, const std::vector<cv::Point2i>& proposal, int direction,
                        bool type) {
    cv::Mat img_show;
    if (img.channels() != 3) {
    ...
    for (int i = 0; i < board.idx.size(); ++i) {
        for (int j = 0; j < board.idx[i].size(); ++j) {
            ...
            cv::putText(img_show, std::to_string(board.idx[i][j]), cv::Point2i(corners.p[board.idx[i][j]].x - 12, corners.p[board.idx[i][j]].y - 6),
    ...
    mean.x -= 10;
    mean.y += 10;
    cv::putText(img_show, std::to_string(direction), mean, cv::FONT_HERSHEY_SIMPLEX, 1.3, cv::Scalar(196, 196, 0), 2);
