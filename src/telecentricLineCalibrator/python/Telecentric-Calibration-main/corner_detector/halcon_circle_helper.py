...
def get_contours(img):
    ...
    threshold = cv2.adaptiveThreshold(
        ...
        cv2.THRESH_BINARY, 125, 2
    )
    # 中值滤波
    median_blurred_image = cv2.medianBlur(threshold, 7)
    # 轮廓提取
    contours, hierarchy = cv2.findContours(
        median_blurred_image, cv2.RETR_TREE, cv2.CHAIN_APPROX_NONE
    )

    if hierarchy is None:
        print("未检测到任何轮廓（可能图像过暗或阈值失败）")
        return None
    ...
    for i, contour in enumerate(contours):
        ...
        if len(approx) == 4:
            ...
            if child_index == -1:
                print("外框找到，但未检测到内部子轮廓（可能内框缺失）")
                continue

            # 取内框
            if hierarchy[0][child_index][0] == -1:
                ...
                if len(approx1) == 5:
                    found_inner = True
                    # 提取圆轮廓
                    n, circle_contours = get_all_child_contours(
                        child_index, contours, hierarchy
                    )
                    if n > 0:
                        found_circles = True
                        return n, approx.reshape(-1, 2), approx1.reshape(-1, 2), circle_contours
                    else:
                        print("外框、内框找到，但未检测到内部圆点")
                        return None

    # 针对不同情况输出更具体的提示
    if not found_outer:
        print("外框定位失败：未检测到四边形外框")
    elif not found_inner:
        print("内框定位失败：外框存在但未检测到五边形内框")
    elif not found_circles:
        print("圆点定位失败：内框存在但未检测到圆形轮廓")
