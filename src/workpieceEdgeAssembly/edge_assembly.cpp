 ...
 * @return 组合是否合法
 */
bool EdgeAssembly::checkWorkpieceIntersections(const std::vector<WorkpieceBoundingBox>& workpieces,
 ...
 */
void EdgeAssembly::generateOptimizedCombinations(int start, int k, std::vector<int>& current, std::set<int>& usedContourIds,
    ...
    int remainingContours = static_cast<int>(m_cbbs.size()) - static_cast<int>(usedContourIds.size());
 ...
 *    - 剪枝5：跳过包含已使用轮廓的工件
 */
void EdgeAssembly::dfsCombinations(const std::vector<WorkpieceInfo>& infos, int start, int k, ContourSet targetMask,
 ...
 *    - 工件之间不相交（避免空间冲突）
 */
void EdgeAssembly::generateValidateCombinations(int n) {
 ...
 * @details
 * 流程说明：
 * 1. generateWorkpiece() - 从轮廓生成候选工件
 * 2. generateValidateCombinations(n) - 生成并验证n个工件的组合
 * 3. calculateMostLikelyCombination() - 计算并选择最优组合
 */
void EdgeAssembly::run(int n) {
    generateWorkpiece();
    generateValidateCombinations(n);
