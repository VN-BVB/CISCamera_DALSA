...
int main(int argc, char *argv[]) {
    ...
    std::unique_ptr<ExternalExeRunner> runner = std::make_unique<ExternalExeRunner>();

    // 添加 DLL 搜索路径
    runner->addDllDirToPath("D:/Code/CISCamera_DALSA/data/CISConfig/externExE");

    // 启动外部 exe（示例 mytool.exe）
    if (runner->start("D:/Code/CISCamera_DALSA/data/CISConfig/externExE/ConfigCIS.exe", {"--help"})) {
